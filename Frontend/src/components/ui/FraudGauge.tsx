...
export function FraudGauge({ score, size = 'md' }: FraudGaugeProps) {
  ...
  const getColor = (s: number) => {
    if (s < 30) return { stroke: '#22c55e', text: 'text-green-600', label: 'Low Risk' }
    if (s < 70) return { stroke: '#f59e0b', text: 'text-amber-600', label: 'Medium Risk' }
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: config.width, height: config.height }}>
        ...
        {/* Score Display - high risk (≥70) EXACTLY text-red-600 */}
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-2">
          <span
            className={cn(
              'font-display font-bold',
              config.fontSize,
              score >= 70 ? 'text-red-600' : color.text
            )}
          >
            {animatedScore}
          </span>
      ...
      <div className="mt-3 text-center">
        ...
        <p className="text-xs text-gray-700 mt-0.5">Fraud Score</p>
      </div>
    </div>
  )
}
