...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  ...
  try {
    ...
    if (error instanceof TypeError && error.message === 'Failed to fetch') {
      console.error('Network error: Backend server may not be running at', API_BASE)
      throw new Error(`Cannot connect to backend server. Please ensure the server is running at ${API_BASE}`)
    }
    throw error
  }
}
